...
namespace BaiduTalk
{
    internal class BassPlayer
    {
        ...
        public BassPlayer(System.IntPtr win,string SoundcardName)
        {
            BassPlayer.pFFT = new int[BassPlayer.c_Index.Length];
            BassNet.Registration("weihong.guan@gmail.com", "2X32231422152222");

            int deviceNO = GetDevicesFromString(SoundcardName);
            if (deviceNO < 0) deviceNO = -1;

            if (Bass.BASS_Init(deviceNO, 44100, BASSInit.BASS_DEVICE_LATENCY, win))
            {
